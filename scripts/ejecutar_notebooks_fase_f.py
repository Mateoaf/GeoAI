"""Ejecuta 10–14 con este intérprete. --desde 11 permite continuar el run actual."""
import argparse
import json
...
def main():
    ...
    parser.add_argument('--desde',type=int,choices=range(10,15),default=10)
    parser.add_argument('--hasta',type=int,choices=range(10,15),default=14)
    ...
    for number in range(args.desde,args.hasta+1):
        ...
        with tempfile.TemporaryDirectory(prefix='geoau-f-kernel-') as directory:
            ...
            (folder/'kernel.json').write_text(json.dumps({'argv':[sys.executable,'-m','ipykernel_launcher','-f','{connection_file}'],
                'display_name':'GeoAu fase F','language':'python'}),encoding='utf-8')
