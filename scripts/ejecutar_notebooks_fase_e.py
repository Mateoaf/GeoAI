"""Ejecuta 07–09 con este intérprete. --desde 8 permite continuar el run actual."""
import argparse
import json
...
def main():
    ...
    parser.add_argument('--desde',type=int,choices=range(7,10),default=7)
    parser.add_argument('--hasta',type=int,choices=range(7,10),default=9)
    ...
    for number in range(args.desde,args.hasta+1):
        ...
        with tempfile.TemporaryDirectory(prefix='geoau-e-kernel-') as directory:
            ...
            (folder/'kernel.json').write_text(json.dumps({'argv':[sys.executable,'-m','ipykernel_launcher','-f','{connection_file}'],
                'display_name':'GeoAu fase E','language':'python'}),encoding='utf-8')
